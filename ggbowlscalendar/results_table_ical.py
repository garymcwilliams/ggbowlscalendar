...
class ResultsTableIcal:
    ...
    def summary(self, result: LeagueResult, opp_team_details: TeamData) -> str:
        ...
        if result.not_played_yet():
            summary = (f"{match_names} {result.label}").rstrip()
        else:
            summary = (
                f"{match_names} "
                f"{result.result} "
    ...
    def match_desc(self, result: LeagueResult, opp_team_details: TeamData) -> str:
        ...
        if result.location:
            ...
            venue = "neutral"
        depart_desc = ""
        if result.is_away() and result.not_played_yet():
            depart_time = result.match_date_time() - timedelta(
                minutes=opp_team_details.depart
            )
            depart_text = datetime.strftime(depart_time, "%H:%M")
            depart_desc = f", depart at:{depart_text}"
        desc = f"{result.result} {venue} ({opp_name}){depart_desc}"
        if desc != desc.strip():
